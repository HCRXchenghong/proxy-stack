...
ShellCheck 0.8.0（排除“被其他 source 调用方使用的全局变量”误报 SC2034）
git diff --check
```

单元测试覆盖 23 项，包括：

- 五协议链接与 Mihomo 禁止 LAN。
- 用户名不可作为公网订阅键。
...
1. 五路协议若部署在同一台服务器、同一公网 IP，仍共享封锁、DDoS、供应商停机和主机失陷故障域。真正的高可用至少需要两台不同供应商、不同 ASN 的服务器。
...
还应从未授权网络测试订阅返回 404，从授权设备测试五路连接，并用云厂商带外控制台确认即使 SSH 被误封仍能恢复。部署前保留 `/etc/proxy-stack` 和项目目录备份。
